...
| 検証番号 | 検証項目 | 結果 | 備考 |
|---|---|---|---|
| WV-00 | Dock埋め込み成立性確認 | 条件付き成功 | Child Window Overlay方式は成立。Dock UIとの共存制御により入力競合は解消 |
| WV-01 | WebView表示 | 成功 | build_as_child(frame) により表示成功 |
| WV-02 | egui共存 | 条件付き成功 | WebView Support Panel成立。フローティングPanelとネイティブサーフェスのZオーダー課題あり |
...
ネイティブサーフェス（WebView / GPU Viewport 等）を Dock Panel 上へ
Child Window Overlay 方式で配置した場合、egui_dock の Floating Panel と
Zオーダー競合が発生する。
...
フローティングPanelは同一 eframe Window 内の描画であり、
独立 HWND を持たない。

そのため Child Window Overlay 方式では
ネイティブサーフェスとの前後関係制御が困難である。
...
WV-02では、egui_dock と WebView の共存が成立した。

また、PoC-2f により Floating Panel の禁止が可能であることを確認した。

ただし Child Window Overlay 方式では、Floating Panel とネイティブサーフェス間の Z オーダー課題が確認されている。

ネイティブサーフェスを含む Panel を使用する場合は、Panel を Dock 内のみ許可する設計が有効な回避策となる。

次工程では、WV-03として複数WebView構成における動作を確認する。

## 次アクション

WV-03:

複数WebView (WebView + GPU Viewport)
