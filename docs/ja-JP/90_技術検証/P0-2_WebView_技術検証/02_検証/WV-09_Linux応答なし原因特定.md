...
P0-2 クロージング時点では、WV-09の切り分け結果により、以下を応答なしの直接原因として扱わない。

- eframe / egui / Dock 単独処理
- Panel同期単独処理
- GTKイベントポンプ単独処理
- Visibility同期単独処理
- WebView set_bounds 単独処理
- WebView set_visible 継続実行
- Native Surface位置同期単独処理

その後の WV-10 / WV-11 により、GTK / WebKitGTK を生成しない構成では応答なしが消失し、GTK / WebKitGTK を表示する構成では応答なしが再現することを確認した。

現時点では、Wayland環境における GTK Window同期方式は採用しない。

本判断は現時点の技術検証結果に基づくものであり、Wayland、winit、eframe、GTK、WebKitGTK など関連技術の進展に応じて将来再評価する。
