...
WV-08-01 ～ WV-08-14 の結果から、以下は応答なしの主因ではない可能性が高い。
...
GTK基盤、WebKitGTK初期化経路、WebView再配置処理、WebView表示有効化処理は正常動作している可能性が高い。

ただし set_visible(false) 経路は未実行である。

問題は set_visible(false)、継続GTKイベントポンプ、または eframe / winit との共存処理に存在する可能性が高い。
...
* set_visible(false) 経路を強制実行し、応答なしが再現するか確認する。
