...
* `WV-08-03 gtk::init success` が出力された。
* `WV-08-03 gtk::Window created` が出力された。
* `WV-08-03 window.show_all done` が出力された。
* GTK Window は表示されなかった。
* PoC-2e / egui_dock は継続動作した。
* Dock操作可能。
* マウス移動可能。
* 応答なしは発生しなかった。
...
GTK Window は表示されなかったが、show_all() 実行自体は成功している。

このため、応答なしの主因は show_all() 単体ではない可能性が高い。
