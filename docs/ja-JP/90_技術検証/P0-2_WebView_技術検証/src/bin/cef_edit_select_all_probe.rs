//! WV-11-04-01 CEF Browser Surface Select All Probe。
//!
//! 役割:
//! - EDIT-IT-SPEC-006 の Select All 操作を検証する。
//! - Browser Surface 内の input 要素を Pointer Button でフォーカスし、Ctrl+A を CEF OSR Browser へ転送する。
//! - Select All 後に input 全体が選択状態になることを Browser 側の selectionStart / selectionEnd で確認する。
//!
//! 注意点:
...
const TEST_TEXT: &str = "SELECT-ALL-12345";
const TEST_URL: &str = "data:text/html,<html><body style='margin:0;background:rgb(26,36,50);color:white;font-family:sans-serif;height:100vh;display:flex;align-items:center;justify-content:center'><div style='text-align:center;width:92%'><h1 style='font-size:38px'>WV-11-04-01 SELECT ALL</h1><div style='font-size:21px;margin:16px'>Click the input to focus it. Then press Ctrl+A.</div><input id='probe' type='text' value='SELECT-ALL-12345' style='width:82%;height:88px;font-size:38px;padding:12px;border:6px solid white;background:rgb(55,78,105);color:white;box-sizing:border-box'><div id='status' style='font-size:28px;margin-top:22px'>SELECTION: waiting</div><div id='selected' style='font-size:24px;margin-top:12px'>TEXT: none</div><div id='result' style='font-size:24px;margin-top:12px'>SELECT ALL: waiting</div></div><script>let p=document.getElementById('probe');let update=function(){let s=p.selectionStart||0;let e=p.selectionEnd||0;let t=p.value.substring(s,e);document.getElementById('status').textContent='SELECTION: '+s+' - '+e;document.getElementById('selected').textContent='TEXT: '+(t||'none');document.getElementById('result').textContent=(s===0&&e===p.value.length&&p.value.length>0)?'SELECT ALL: fired':'SELECT ALL: waiting';};p.addEventListener('focus',update);p.addEventListener('select',update);p.addEventListener('keyup',update);document.addEventListener('selectionchange',update);</script></body></html>";

/// 最新 CEF Paint を保持する共有状態。
///
/// @hldocs.ref doc-20260912-104800Z-WV15#sec_c8x4p1m6r9bz
...
#[derive(Clone)]
struct SelectAllRenderHandler {
    state: Arc<Mutex<SelectAllState>>,
}

wrap_render_handler! {
    struct SelectAllRenderHandlerBuilder {
        handler: SelectAllRenderHandler,
    }
...
impl SelectAllRenderHandlerBuilder {
    fn build(state: Arc<Mutex<SelectAllState>>) -> RenderHandler {
        Self::new(SelectAllRenderHandler { state })
    }
}

#[derive(Clone)]
struct SelectAllLifeSpanHandler {
...
wrap_life_span_handler! {
    struct SelectAllLifeSpanHandlerBuilder {
        handler: SelectAllLifeSpanHandler,
    }
...
impl SelectAllLifeSpanHandlerBuilder {
    fn build(
        browser: Arc<Mutex<Option<Browser>>>,
        browser_created: Arc<AtomicBool>,
        closed: Arc<AtomicBool>,
    ) -> LifeSpanHandler {
        Self::new(SelectAllLifeSpanHandler {
            browser,
            browser_created,
            closed,
        })
...
impl SelectAllClientBuilder {
    fn build(
        ...
        Self::new(
            SelectAllRenderHandlerBuilder::build(state),
            SelectAllLifeSpanHandlerBuilder::build(browser, browser_created, closed),
        )
    }
}

#[derive(Clone)]
struct SelectAllBrowserProcessHandler {
    state: Arc<Mutex<SelectAllState>>,
    browser: Arc<Mutex<Option<Browser>>>,
    browser_created: Arc<AtomicBool>,
...
wrap_browser_process_handler! {
    struct SelectAllBrowserProcessHandlerBuilder {
        handler: SelectAllBrowserProcessHandler,
    }

    impl BrowserProcessHandler {
        fn on_context_initialized(&self) {
            let mut client = SelectAllClientBuilder::build(
...
impl SelectAllBrowserProcessHandlerBuilder {
    fn build(handler: SelectAllBrowserProcessHandler) -> BrowserProcessHandler {
        Self::new(handler)
    }
}

#[derive(Clone)]
struct SelectAllCefApp {
    browser_process_handler: BrowserProcessHandler,
}

wrap_app! {
    struct SelectAllCefAppBuilder {
        handler: SelectAllCefApp,
    }
...
impl SelectAllCefAppBuilder {
    fn build(handler: SelectAllBrowserProcessHandler) -> App {
        Self::new(SelectAllCefApp {
            browser_process_handler: SelectAllBrowserProcessHandlerBuilder::build(handler),
        })
    }
}
...
fn run_subprocess() -> i32 {
    ...
    if exit_code >= 0 {
        ...
        eprintln!("WV-11-04-01 select all subprocess failed: cef_execute_process returned {exit_code}");
        1
    }
}
...
impl<'a> TabViewer for SelectAllViewer<'a> {
    ...
    fn title(&mut self, _tab: &mut Self::Tab) -> egui::WidgetText {
        "Browser Surface Select All".into()
    }
...
struct SelectAllRuntime {
    state: Arc<Mutex<SelectAllState>>,
    browser: Arc<Mutex<Option<Browser>>>,
    closed: Arc<AtomicBool>,
}

impl SelectAllRuntime {
    fn new() -> Result<Self, String> {
        ...
        let mut app = SelectAllCefAppBuilder::build(handler);
    ...
    fn request_select_all_key(&self, transfer: SelectAllKeyTransfer) -> bool {
        let Some(browser) = self.browser.lock().ok().and_then(|slot| slot.as_ref().cloned()) else {
            return false;
        };
        #[cfg(target_os = "windows")]
        {
            let mut task = SelectAllKeyTaskBuilder::build(browser, transfer);
            return post_task(ThreadId::UI, Some(&mut task)) == 1;
        }
        #[cfg(not(target_os = "windows"))]
        {
            send_select_all_key(&browser, transfer);
            true
        }
    }
}
...
fn send_click(browser: &Browser, transfer: ClickTransfer) {
    ...
    println!("CEF select all focus click sent: x={}, y={}", transfer.x, transfer.y);
}

/// Windows の A key に対応する native_key_code を返す。
///
/// # 引数
/// - `pressed`: key-down の場合 `true`、key-up の場合 `false`。
///
/// # 戻り値
/// - Win32 の WM_KEYDOWN / WM_KEYUP で A key を受けた場合の lParam 相当値。
#[cfg(target_os = "windows")]
fn native_key_code_a(pressed: bool) -> i32 {
    const SCAN_CODE_A: i32 = 0x1E;
    let mut value = 1 | (SCAN_CODE_A << 16);
    if !pressed {
        value |= 1 << 30;
...
#[cfg(not(target_os = "windows"))]
fn native_key_code_a(_pressed: bool) -> i32 {
    0x41
}

/// CEF BrowserHost へ Ctrl+A を送信する。
///
/// @hldocs.ref doc-20260912-104800Z-WV15#sec_c8x4p1m6r9bz
fn send_select_all_key(browser: &Browser, transfer: SelectAllKeyTransfer) {
    let Some(host) = browser.host() else {
        return;
    };
    let event = KeyEvent {
        ...
        native_key_code: native_key_code_a(transfer.pressed),
        modifiers: CEF_EVENTFLAG_CONTROL_DOWN,
        ..Default::default()
    };
    host.send_key_event(Some(&event));
    println!(
        "CEF select all key sent: {} key=A vk=65 native={} ctrl=true",
...
#[derive(Clone)]
struct SelectAllKeyTask {
    browser: Browser,
    transfer: SelectAllKeyTransfer,
}

wrap_task! {
    struct SelectAllKeyTaskBuilder {
        task: SelectAllKeyTask,
    }

    impl Task {
        fn execute(&self) {
            send_select_all_key(&self.task.browser, self.task.transfer);
        }
    }
}

impl SelectAllKeyTaskBuilder {
    fn build(browser: Browser, transfer: SelectAllKeyTransfer) -> Task {
        Self::new(SelectAllKeyTask { browser, transfer })
    }
}
...
impl Drop for SelectAllRuntime {
    fn drop(&mut self) {
        ...
        if let Some(browser) = browser {
            #[cfg(target_os = "windows")]
            {
                ...
                if post_task(ThreadId::UI, Some(&mut task)) != 1 {
                    eprintln!("WV-11-04-01 select all probe: failed to post browser close task");
                }
            }
            #[cfg(not(target_os = "windows"))]
        ...
        if !self.closed.load(Ordering::Acquire) {
            eprintln!("WV-11-04-01 select all probe: browser close timeout");
        }
        shutdown();
    }
}

struct SelectAllEframeApp {
    dock_state: DockState<SelectAllTab>,
    runtime: SelectAllRuntime,
...
impl SelectAllEframeApp {
    fn new(runtime: SelectAllRuntime) -> Self {
        Self {
            dock_state: DockState::new(vec![SelectAllTab::Browser]),
            runtime,
            texture: None,
    ...
    /// egui の Ctrl+A Key Event を CEF へ転送する。
    fn collect_select_all_key(&mut self, ctx: &egui::Context) {
        ...
        for event in events {
            ...
            if self.runtime.request_select_all_key(SelectAllKeyTransfer { pressed }) {
...
impl eframe::App for SelectAllEframeApp {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        egui::TopBottomPanel::top("wv11_04_01_select_all_status").show(ctx, |ui| {
            ui.horizontal_wrapped(|ui| {
                ui.label("WV-11-04-01 EDIT-IT-SPEC-006");
                ui.separator();
                ...
                ui.separator();
                ui.label(format!("Expected selection: {TEST_TEXT}"));
            });
        });

        self.current_click = None;
        egui::CentralPanel::default().show(ctx, |ui| {
            let mut viewer = SelectAllViewer {
        ...
        self.collect_select_all_key(ctx);
...
fn main() -> eframe::Result<()> {
    ...
    println!("WV-11-04-01 CEF select all probe start");
    println!("Expected Browser selection after Ctrl+A: {TEST_TEXT}");
    let runtime = match SelectAllRuntime::new() {
        Ok(runtime) => runtime,
        Err(error) => {
            eprintln!("WV-11-04-01 CEF select all probe failed: {error}");
            std::process::exit(1);
        }
    };
    ...
    let native_options = eframe::NativeOptions {
        viewport: egui::ViewportBuilder::default()
            .with_title("WV-11-04-01 Browser Surface Select All Probe")
            .with_inner_size([1000.0, 720.0]),
        ..Default::default()
    };

    eframe::run_native(
        "WV-11-04-01 Browser Surface Select All Probe",
        native_options,
        Box::new(move |_cc| Ok(Box::new(SelectAllEframeApp::new(runtime)))),
    )
}
