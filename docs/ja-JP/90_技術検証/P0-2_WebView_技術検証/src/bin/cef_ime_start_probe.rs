//! WV-11-04-02 CEF Browser Surface IME Start Probe。
//!
//! 役割:
//! - IME-IT-SPEC-001 の IME 入力開始を検証する。
//! - Browser Surface 内の input 要素を Pointer Button でフォーカスする。
//! - eframe/egui で受信した IME Preedit を CEF OSR Browser の Composition 開始へ転送する。
//! - Browser 側で Composition が開始し、未確定文字列が表示されることを確認する。
//!
//! 注意点:
...
//! - 本 Probe は IME 開始のみを対象とし、Commit / Cancel / Candidate 位置同期は後続検証で扱う。
...
const TEST_URL: &str = "data:text/html,<html><body style='margin:0;background:rgb(26,36,50);color:white;font-family:sans-serif;height:100vh;display:flex;align-items:center;justify-content:center'><div style='text-align:center;width:92%'><h1 style='font-size:38px'>WV-11-04-02 IME START</h1><div style='font-size:21px;margin:16px'>Click the input, enable the OS IME, then start composing text without confirming it.</div><input id='probe' type='text' value='' style='width:82%;height:88px;font-size:38px;padding:12px;border:6px solid white;background:rgb(55,78,105);color:white;box-sizing:border-box'><div id='start' style='font-size:28px;margin-top:22px'>COMPOSITION START: waiting</div><div id='update' style='font-size:24px;margin-top:12px'>COMPOSITION TEXT: none</div><div id='value' style='font-size:24px;margin-top:12px'>VALUE: empty</div></div><script>let p=document.getElementById('probe');let start=document.getElementById('start');let update=document.getElementById('update');let value=document.getElementById('value');p.addEventListener('compositionstart',function(){start.textContent='COMPOSITION START: fired';});p.addEventListener('compositionupdate',function(e){update.textContent='COMPOSITION TEXT: '+(e.data||'<empty>');value.textContent='VALUE: '+(p.value||'<empty>');});p.addEventListener('input',function(){value.textContent='VALUE: '+(p.value||'<empty>');});</script></body></html>";

/// 最新 CEF Paint を保持する共有状態。
///
/// @hldocs.ref doc-20260912-104801Z-WV16#sec_n4q8c2v7m1kt
#[derive(Debug, Default)]
struct ImeStartState {
    width: i32,
    height: i32,
    rgba: Vec<u8>,
    generation: u64,
...
#[derive(Clone)]
struct ImeStartRenderHandler {
    state: Arc<Mutex<ImeStartState>>,
}

wrap_render_handler! {
    struct ImeStartRenderHandlerBuilder {
        handler: ImeStartRenderHandler,
    }
...
impl ImeStartRenderHandlerBuilder {
    fn build(state: Arc<Mutex<ImeStartState>>) -> RenderHandler {
        Self::new(ImeStartRenderHandler { state })
    }
}

#[derive(Clone)]
struct ImeStartLifeSpanHandler {
...
wrap_life_span_handler! {
    struct ImeStartLifeSpanHandlerBuilder {
        handler: ImeStartLifeSpanHandler,
    }
...
impl ImeStartLifeSpanHandlerBuilder {
    fn build(
        browser: Arc<Mutex<Option<Browser>>>,
        browser_created: Arc<AtomicBool>,
        closed: Arc<AtomicBool>,
    ) -> LifeSpanHandler {
        Self::new(ImeStartLifeSpanHandler {
            browser,
            browser_created,
            closed,
        })
...
impl ImeStartClientBuilder {
    fn build(
        ...
        Self::new(
            ImeStartRenderHandlerBuilder::build(state),
            ImeStartLifeSpanHandlerBuilder::build(browser, browser_created, closed),
        )
    }
}

#[derive(Clone)]
struct ImeStartBrowserProcessHandler {
    state: Arc<Mutex<ImeStartState>>,
    browser: Arc<Mutex<Option<Browser>>>,
    browser_created: Arc<AtomicBool>,
...
wrap_browser_process_handler! {
    struct ImeStartBrowserProcessHandlerBuilder {
        handler: ImeStartBrowserProcessHandler,
    }

    impl BrowserProcessHandler {
        fn on_context_initialized(&self) {
            let mut client = ImeStartClientBuilder::build(
...
impl ImeStartBrowserProcessHandlerBuilder {
    fn build(handler: ImeStartBrowserProcessHandler) -> BrowserProcessHandler {
        Self::new(handler)
    }
}

#[derive(Clone)]
struct ImeStartCefApp {
    browser_process_handler: BrowserProcessHandler,
}

wrap_app! {
    struct ImeStartCefAppBuilder {
        handler: ImeStartCefApp,
    }
...
impl ImeStartCefAppBuilder {
    fn build(handler: ImeStartBrowserProcessHandler) -> App {
        Self::new(ImeStartCefApp {
            browser_process_handler: ImeStartBrowserProcessHandlerBuilder::build(handler),
        })
    }
}
...
fn run_subprocess() -> i32 {
    ...
    if exit_code >= 0 {
        ...
        eprintln!(
            "WV-11-04-02 IME start subprocess failed: cef_execute_process returned {exit_code}"
        );
        1
    }
}
...
struct ImeStartViewer<'a> {
    texture: Option<&'a egui::TextureHandle>,
    click_transfer: &'a mut Option<ClickTransfer>,
    browser_active: &'a mut bool,
    ime_rect: &'a mut Option<egui::Rect>,
}

impl<'a> TabViewer for ImeStartViewer<'a> {
    type Tab = ImeStartTab;
    ...
    fn title(&mut self, _tab: &mut Self::Tab) -> egui::WidgetText {
        "Browser Surface IME Start".into()
    }
    ...
    fn ui(&mut self, ui: &mut egui::Ui, _tab: &mut Self::Tab) {
        ...
        *self.ime_rect = Some(response.rect);
...
struct ImeStartRuntime {
    state: Arc<Mutex<ImeStartState>>,
    browser: Arc<Mutex<Option<Browser>>>,
    closed: Arc<AtomicBool>,
}

impl ImeStartRuntime {
    fn new() -> Result<Self, String> {
        ...
        let mut app = ImeStartCefAppBuilder::build(handler);
...
/// Browser 内の editable 要素へ Pointer Click を転送する。
fn send_click(browser: &Browser, transfer: ClickTransfer) {
    ...
    println!(
        "CEF IME start focus click sent: x={}, y={}",
        transfer.x, transfer.y
    );
}

/// CEF BrowserHost へ IME Preedit を Composition として送信する。
///
/// # 引数
/// - `browser`: 対象 CEF Browser。
/// - `transfer`: OS IME から受信した未確定文字列。
///
/// # 戻り値
/// - なし。
///
...
fn send_ime_preedit(browser: &Browser, transfer: &ImePreeditTransfer) {
    ...
    host.ime_set_composition(Some(&text), None, None, None);
    println!("CEF IME composition set: {:?}", transfer.text);
}
...
impl Drop for ImeStartRuntime {
    fn drop(&mut self) {
        ...
        if let Some(browser) = browser {
            #[cfg(target_os = "windows")]
            {
                ...
                if post_task(ThreadId::UI, Some(&mut task)) != 1 {
                    eprintln!("WV-11-04-02 IME start probe: failed to post browser close task");
                }
            }
        ...
        if !self.closed.load(Ordering::Acquire) {
            eprintln!("WV-11-04-02 IME start probe: browser close timeout");
        }
        shutdown();
    }
}

struct ImeStartEframeApp {
    dock_state: DockState<ImeStartTab>,
    runtime: ImeStartRuntime,
...
impl ImeStartEframeApp {
    fn new(runtime: ImeStartRuntime) -> Self {
        Self {
            dock_state: DockState::new(vec![ImeStartTab::Browser]),
            runtime,
            texture: None,
    ...
    /// egui の IME Preedit Event を CEF Composition 開始へ転送する。
        ...
        for event in events {
            ...
            match ime_event {
                ...
                egui::ImeEvent::Commit(text) => {
                    println!("egui IME commit observed but not forwarded in start probe: {:?}", text);
...
impl eframe::App for ImeStartEframeApp {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        if self.browser_active {
            ...
            if let Some(rect) = self.ime_rect {
                ctx.send_viewport_cmd(egui::ViewportCommand::IMERect(rect));
            }
        }
        ...
        egui::TopBottomPanel::top("wv11_04_02_ime_start_status").show(ctx, |ui| {
            ui.horizontal_wrapped(|ui| {
                ui.label("WV-11-04-02 IME-IT-SPEC-001");
                ui.separator();
...
fn main() -> eframe::Result<()> {
    ...
    println!("WV-11-04-02 CEF IME start probe start");
    println!("Click Browser input, enable OS IME, and begin composition without confirming it.");

    let runtime = match ImeStartRuntime::new() {
        Ok(runtime) => runtime,
        Err(error) => {
            eprintln!("WV-11-04-02 CEF IME start probe failed: {error}");
            std::process::exit(1);
        }
    };
    ...
    let native_options = eframe::NativeOptions {
        viewport: egui::ViewportBuilder::default()
            .with_title("WV-11-04-02 Browser Surface IME Start Probe")
            .with_inner_size([1000.0, 720.0]),
        ..Default::default()
    };

    eframe::run_native(
        "WV-11-04-02 Browser Surface IME Start Probe",
        native_options,
        Box::new(move |_cc| Ok(Box::new(ImeStartEframeApp::new(runtime)))),
    )
}
