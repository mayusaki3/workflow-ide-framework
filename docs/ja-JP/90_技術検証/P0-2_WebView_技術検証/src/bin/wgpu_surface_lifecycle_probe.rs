...
impl ProbeApp {
    fn new(cc: &eframe::CreationContext<'_>) -> Self {
        let render_state = cc
            .wgpu_render_state
            .as_ref()
            .expect("WV-12-03 requires the eframe wgpu renderer")
            .clone();
    ...
    fn destroy_gpu(&mut self) {
        if let Some(gpu) = self.gpu.take() {
            ...
            println!(
                "GPU Surface destroy: instance={}, final_generation={}, size={}x{}",
    ...
    fn render_gpu_frame(&mut self) {
        ...
        if gpu.generation == 1 || gpu.generation % 120 == 0 {
            println!(
                "GPU Surface frame: instance={}, generation={}, size={}x{}",
                gpu.instance, gpu.generation, gpu.width, gpu.height
            );
        }
    }
...
impl Drop for ProbeApp {
    fn drop(&mut self) {
        self.destroy_gpu();
        println!("WV-12-03 GPU Surface lifecycle probe shutdown");
    }
}
...
struct ProbeTabViewer<'a> {
    ...
    instance: u64,
    visible: &'a mut bool,
    requested_size: &'a mut [u32; 2],
    destroy_requested: &'a mut bool,
    create_requested: &'a mut bool,
}
...
impl TabViewer for ProbeTabViewer<'_> {
    ...
    fn title(&mut self, tab: &mut Self::Tab) -> egui::WidgetText {
        match tab {
            ...
            ProbeTab::Info => "Lifecycle Control".into(),
        }
    }
    ...
    fn ui(&mut self, ui: &mut egui::Ui, tab: &mut Self::Tab) {
        match tab {
            ...
            ProbeTab::Info => {
                ...
                if let Some([width, height]) = self.texture_size {
                    ui.label(format!("Texture size: {width}x{height}"));
                } else {
                    ui.label("Texture size: none");
                }
                ui.label(format!(
                    "Requested Dock size: {}x{}",
                    self.requested_size[0], self.requested_size[1]
                ));
                ui.separator();
                ...
                if ui.button("Destroy GPU Surface").clicked() {
                    *self.destroy_requested = true;
                }
                if ui.button("Create / Recreate GPU Surface").clicked() {
                ...
                ui.label("3. Destroy 後に Surface が消え、Create / Recreate で再生成できること");
                ui.label("4. 再生成時に instance が増え、generation が 1 から再開すること");
...
impl eframe::App for ProbeApp {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        let texture_id = self.gpu.as_ref().map(|gpu| gpu.texture_id);
        let texture_size = self.gpu.as_ref().map(|gpu| [gpu.width, gpu.height]);
        let generation = self.gpu.as_ref().map_or(0, |gpu| gpu.generation);
        let instance = self.gpu.as_ref().map_or(0, |gpu| gpu.instance);
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            let mut viewer = ProbeTabViewer {
                ...
                destroy_requested: &mut destroy_requested,
                create_requested: &mut create_requested,
            };
...
fn main() -> eframe::Result<()> {
    let options = eframe::NativeOptions {
        ...
        viewport: egui::ViewportBuilder::default()
            .with_title("WV-12-03 GPU Surface Lifecycle Probe")
            .with_inner_size([1000.0, 720.0]),
        ..Default::default()
    };

    eframe::run_native(
        "WV-12-03 GPU Surface Lifecycle Probe",
        options,
