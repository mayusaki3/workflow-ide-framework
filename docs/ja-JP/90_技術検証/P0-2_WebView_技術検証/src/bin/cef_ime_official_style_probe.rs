//! WV-11-04-02 CEF Browser Surface IME Official-style Probe。
//!
//! 役割:
//! - IME-IT-SPEC-001 / 002 の成立条件を、CEF 公式 Windows OSR 実装に近い形で切り分ける。
//! - BrowserHost focus を明示し、Composition 全体を selection range として渡す。
//! - Composition 全体を覆う underline を付与して `ime_set_composition` を呼び出す。
//! - Browser 内の未確定文字列表示と `on_ime_composition_range_changed` 発火を確認する。
//!
//! 注意点:
//! - 本ファイルは技術検証用であり、正式 Surface API ではない。
//! - OS IME の clause/target 情報そのものはまだ取得せず、既定 underline 1 本で切り分ける。
//! - Candidate Window の位置同期、Commit / Cancel は後続検証で扱う。
...
impl<'a> TabViewer for ProbeViewer<'a> {
    ...
    fn title(&mut self, _tab: &mut Self::Tab) -> egui::WidgetText {
        "Browser Surface IME Official Style".into()
    }
...
fn send_click(browser: &Browser, transfer: ClickTransfer) {
    ...
    println!(
        "CEF IME official-style focus click sent: x={}, y={}",
        transfer.x, transfer.y
    );
}

/// CEF 公式 Windows OSR 実装に近い形で IME Preedit を送信する。
///
/// Composition 全体を selection range とし、全体を覆う underline を 1 本付与する。
/// OS IME の clause/target 情報はこの Probe では未取得のため、既定 underline で代替する。
///
...
fn send_ime_preedit(browser: &Browser, transfer: &ImePreeditTransfer) {
    ...
    println!(
        "CEF IME official-style composition set: {:?} selection={}..{} underlines={}",
        transfer.text,
        selection.from,
        selection.to,
        underlines.len()
...
impl Drop for ProbeRuntime {
    fn drop(&mut self) {
        ...
        if let Some(browser) = browser {
            #[cfg(target_os = "windows")]
            {
                ...
                if post_task(ThreadId::UI, Some(&mut task)) != 1 {
                    eprintln!("WV-11-04-02 IME official-style probe: failed to post browser close task");
                }
            }
            #[cfg(not(target_os = "windows"))]
        ...
        if !self.closed.load(Ordering::Acquire) {
            eprintln!("WV-11-04-02 IME official-style probe: browser close timeout");
        }
        shutdown();
    }
}
        ...
        match self.texture.as_mut() {
            ...
            _ => {
                self.texture = Some(ctx.load_texture(
                    "wv11_04_02_ime_official_style_probe",
                    image,
        ...
        for event in events {
            ...
            match ime_event {
                ...
                egui::ImeEvent::Commit(text) => {
                    println!("egui IME commit observed but not forwarded in official-style probe: {:?}", text);
...
impl eframe::App for ProbeEframeApp {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        egui::TopBottomPanel::top("wv11_04_02_ime_official_style_status").show(ctx, |ui| {
            ui.horizontal_wrapped(|ui| {
                ui.label("WV-11-04-02 IME Official-style Probe");
                ui.separator();
...
fn main() -> eframe::Result<()> {
    ...
    println!("WV-11-04-02 CEF IME official-style probe start");
    println!("Click Browser input, enable OS IME, and type without confirming.");

    let runtime = match ProbeRuntime::new() {
        Ok(runtime) => runtime,
        Err(error) => {
            eprintln!("WV-11-04-02 CEF IME official-style probe failed: {error}");
            std::process::exit(1);
        }
    };
    ...
    let native_options = eframe::NativeOptions {
        viewport: egui::ViewportBuilder::default()
            .with_title("WV-11-04-02 Browser Surface IME Official-style Probe")
            .with_inner_size([1000.0, 720.0]),
        ..Default::default()
    };

    eframe::run_native(
        "WV-11-04-02 Browser Surface IME Official-style Probe",
        native_options,
