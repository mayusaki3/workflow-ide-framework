//! WV-11-04-01 CEF Browser Surface Keyboard 範囲選択 Probe。
//!
//! 役割:
//! - EDIT-IT-SPEC-001 の Keyboard 範囲選択を検証する。
//! - Browser Surface 内の input 要素へ Pointer Button でフォーカスした後、
//!   Shift+ArrowLeft / Shift+ArrowRight を CEF OSR Browser へ転送する。
//! - Browser 側で selectionStart / selectionEnd を表示し、選択範囲を視覚確認する。
//!
//! 注意点:
//! - 本ファイルは技術検証用であり、正式 Surface API ではない。
//! - Windows Virtual-Key Code と CEF EVENTFLAG_SHIFT_DOWN の使用は本 Probe の検証実装である。
...
const TEST_URL: &str = "data:text/html,<html><body style='margin:0;background:rgb(26,36,50);color:white;font-family:sans-serif;height:100vh;display:flex;align-items:center;justify-content:center'><div style='text-align:center;width:92%'><h1 style='font-size:38px'>WV-11-04-01 KEYBOARD SELECTION</h1><div style='font-size:21px;margin:16px'>Click inside the text, then hold Shift and press Left / Right.</div><input id='probe' type='text' value='SELECT-ME-12345' style='width:82%;height:88px;font-size:38px;padding:12px;border:6px solid white;background:rgb(55,78,105);color:white;box-sizing:border-box'><div id='status' style='font-size:28px;margin-top:22px'>SELECTION: waiting</div><div id='selected' style='font-size:24px;margin-top:12px'>TEXT: none</div></div><script>let p=document.getElementById('probe');let update=function(){let s=p.selectionStart||0;let e=p.selectionEnd||0;document.getElementById('status').textContent='SELECTION: '+s+' - '+e;document.getElementById('selected').textContent='TEXT: '+(p.value.substring(s,e)||'none');};p.addEventListener('focus',update);p.addEventListener('select',update);p.addEventListener('keyup',update);p.addEventListener('click',update);</script></body></html>";

/// 最新 CEF Paint を保持する共有状態。
///
/// @hldocs.ref doc-20260912-104800Z-WV15#sec_f3n8q2v6m1ka
#[derive(Debug, Default)]
struct KeySelectState {
    width: i32,
    height: i32,
    rgba: Vec<u8>,
    generation: u64,
...
#[derive(Clone)]
struct KeySelectRenderHandler {
    state: Arc<Mutex<KeySelectState>>,
}

wrap_render_handler! {
    struct KeySelectRenderHandlerBuilder {
        handler: KeySelectRenderHandler,
    }
...
impl KeySelectRenderHandlerBuilder {
    fn build(state: Arc<Mutex<KeySelectState>>) -> RenderHandler {
        Self::new(KeySelectRenderHandler { state })
    }
}

#[derive(Clone)]
struct KeySelectLifeSpanHandler {
...
wrap_life_span_handler! {
    struct KeySelectLifeSpanHandlerBuilder {
        handler: KeySelectLifeSpanHandler,
    }
...
impl KeySelectLifeSpanHandlerBuilder {
    fn build(
        browser: Arc<Mutex<Option<Browser>>>,
        browser_created: Arc<AtomicBool>,
        closed: Arc<AtomicBool>,
    ) -> LifeSpanHandler {
        Self::new(KeySelectLifeSpanHandler {
            browser,
            browser_created,
            closed,
        })
...
impl KeySelectClientBuilder {
    fn build(
        ...
        Self::new(
            KeySelectRenderHandlerBuilder::build(state),
            KeySelectLifeSpanHandlerBuilder::build(browser, browser_created, closed),
        )
    }
}

#[derive(Clone)]
struct KeySelectBrowserProcessHandler {
    state: Arc<Mutex<KeySelectState>>,
    browser: Arc<Mutex<Option<Browser>>>,
    browser_created: Arc<AtomicBool>,
...
wrap_browser_process_handler! {
    struct KeySelectBrowserProcessHandlerBuilder {
        handler: KeySelectBrowserProcessHandler,
    }

    impl BrowserProcessHandler {
        fn on_context_initialized(&self) {
            let mut client = KeySelectClientBuilder::build(
...
impl KeySelectBrowserProcessHandlerBuilder {
    fn build(handler: KeySelectBrowserProcessHandler) -> BrowserProcessHandler {
        Self::new(handler)
    }
}

#[derive(Clone)]
struct KeySelectCefApp {
    browser_process_handler: BrowserProcessHandler,
}

wrap_app! {
    struct KeySelectCefAppBuilder {
        handler: KeySelectCefApp,
    }
...
impl KeySelectCefAppBuilder {
    fn build(handler: KeySelectBrowserProcessHandler) -> App {
        Self::new(KeySelectCefApp {
            browser_process_handler: KeySelectBrowserProcessHandlerBuilder::build(handler),
        })
    }
}
...
fn run_subprocess() -> i32 {
    ...
    if exit_code >= 0 {
        ...
        eprintln!("WV-11-04-01 keyboard selection subprocess failed: cef_execute_process returned {exit_code}");
        1
    }
}
...
impl<'a> TabViewer for KeySelectViewer<'a> {
    ...
    fn title(&mut self, _tab: &mut Self::Tab) -> egui::WidgetText {
        "Browser Surface Keyboard Selection".into()
    }
...
struct KeySelectRuntime {
    state: Arc<Mutex<KeySelectState>>,
    browser: Arc<Mutex<Option<Browser>>>,
    closed: Arc<AtomicBool>,
}

impl KeySelectRuntime {
    fn new() -> Result<Self, String> {
        ...
        let mut app = KeySelectCefAppBuilder::build(handler);
    ...
    /// Shift を含む移動キー Event を CEF Browser へ転送する。
    fn request_selection_key(&self, transfer: SelectionKeyTransfer) -> bool {
        let Some(browser) = self
            .browser
            .lock()
            .ok()
        ...
        #[cfg(target_os = "windows")]
        {
            let mut task = SelectionKeyTaskBuilder::build(browser, transfer);
            return post_task(ThreadId::UI, Some(&mut task)) == 1;
        }

        #[cfg(not(target_os = "windows"))]
        {
            send_selection_key(&browser, transfer);
            true
        }
    }
}
...
fn send_click(browser: &Browser, transfer: ClickTransfer) {
    ...
    println!("CEF selection focus click sent: x={}, y={}", transfer.x, transfer.y);
}

/// CEF BrowserHost へ Shift modifier を付与した移動キーを送信する。
///
/// @hldocs.ref doc-20260912-104800Z-WV15#sec_f3n8q2v6m1ka
fn send_selection_key(browser: &Browser, transfer: SelectionKeyTransfer) {
    let Some(host) = browser.host() else {
        return;
    };
    ...
    println!(
        "CEF selection key sent: {} key={} vk={} shift={}",
...
#[derive(Clone)]
struct SelectionKeyTask {
    browser: Browser,
    transfer: SelectionKeyTransfer,
}

wrap_task! {
    struct SelectionKeyTaskBuilder {
        task: SelectionKeyTask,
    }

    impl Task {
        fn execute(&self) {
            send_selection_key(&self.task.browser, self.task.transfer);
        }
    }
}

impl SelectionKeyTaskBuilder {
    fn build(browser: Browser, transfer: SelectionKeyTransfer) -> Task {
        Self::new(SelectionKeyTask { browser, transfer })
    }
}
...
impl Drop for KeySelectRuntime {
    fn drop(&mut self) {
        let browser = self
            .browser
            .lock()
        ...
        if let Some(browser) = browser {
            #[cfg(target_os = "windows")]
            {
                ...
                if post_task(ThreadId::UI, Some(&mut task)) != 1 {
                    eprintln!("WV-11-04-01 keyboard selection probe: failed to post browser close task");
                }
            }
        ...
        if !self.closed.load(Ordering::Acquire) {
            eprintln!("WV-11-04-01 keyboard selection probe: browser close timeout");
        }
        shutdown();
    }
}

struct KeySelectEframeApp {
    dock_state: DockState<KeySelectTab>,
    runtime: KeySelectRuntime,
...
impl KeySelectEframeApp {
    fn new(runtime: KeySelectRuntime) -> Self {
        Self {
            dock_state: DockState::new(vec![KeySelectTab::Browser]),
            runtime,
            texture: None,
    ...
    /// egui の Shift+Arrow Event を検出して CEF へ転送する。
    fn collect_selection_keys(&mut self, ctx: &egui::Context) {
        ...
        for event in events {
            ...
            let mapped = match key {
                egui::Key::ArrowLeft => Some((0x25, "ArrowLeft")),
                egui::Key::ArrowRight => Some((0x27, "ArrowRight")),
                _ => None,
            };
            ...
            let transfer = SelectionKeyTransfer {
                windows_key_code,
                pressed,
                shift: modifiers.shift,
                name,
            };
            if self.runtime.request_selection_key(transfer) {
...
impl eframe::App for KeySelectEframeApp {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        egui::TopBottomPanel::top("wv11_04_01_key_select_status").show(ctx, |ui| {
            ui.horizontal_wrapped(|ui| {
                ui.label("WV-11-04-01 EDIT-IT-SPEC-001");
                ui.separator();
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            let mut viewer = KeySelectViewer {
        ...
        self.collect_selection_keys(ctx);
...
fn main() -> eframe::Result<()> {
    ...
    println!("WV-11-04-01 CEF keyboard selection probe start");
    let runtime = match KeySelectRuntime::new() {
        Ok(runtime) => runtime,
        Err(error) => {
            eprintln!("WV-11-04-01 CEF keyboard selection probe failed: {error}");
            std::process::exit(1);
        }
    };
    ...
    let native_options = eframe::NativeOptions {
        viewport: egui::ViewportBuilder::default()
            .with_title("WV-11-04-01 Browser Surface Keyboard Selection Probe")
            .with_inner_size([1000.0, 720.0]),
        ..Default::default()
    };

    eframe::run_native(
        "WV-11-04-01 Browser Surface Keyboard Selection Probe",
        native_options,
        Box::new(move |_cc| Ok(Box::new(KeySelectEframeApp::new(runtime)))),
    )
}
