...
//! - Dummy GTK Widget生成、継続的なGTKイベント処理、Dock追従move/resizeは実行しない。
//! - WebKitGTK生成 + GtkFixed attach だけで応答なしが発生するか確認する。
//!
//! 注意:
//! - 技術検証用コード。
...
/// - WV-08-10では WebViewのDock追従、Dummy GTK Widget、継続的な GTKイベントポンプは使用しない。
...
///
/// 役割:
/// - WV-08-10では initialize_root_window() 内で WebKitGTK WebView build_gtk のみを確認する。
///
/// 注意:
