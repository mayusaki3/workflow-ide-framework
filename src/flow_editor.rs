...
fn port_position(rect: egui::Rect, node: &FlowNode, port_id: &str, node_size: egui::Vec2) -> Option<egui::Pos2> {
    ...
    for port in &node.ports {
        let p = match port.direction {
            PortDirection::Input => { let p=egui::pos2(node_rect.left(), input_y); input_y+=18.0; p }
            PortDirection::Output => { let p=egui::pos2(node_rect.right(), output_y); output_y+=18.0; p }
        };
...
fn show_canvas(ui: &mut egui::Ui, model: &mut FlowModel, validator: Option<&ConnectionValidator>) -> FlowResponse {
    ...
    if let (Some(start), Some(pointer)) = (&model.pending_connection, ui.input(|i| i.pointer.hover_pos())) {
        if let Some(node) = model.nodes.iter().find(|n| n.id == start.node_id) {
            if let Some(a) = port_position(rect, node, &start.port_id, node_size) {
    ...
    for node in &mut model.nodes {
        ...
        let mut iy=node_rect.top()+34.0; let mut oy=node_rect.top()+34.0;
        for port in &node.ports {
            let p=match port.direction {
                PortDirection::Input=>{let p=egui::pos2(node_rect.left(),iy);iy+=18.0;p}
                PortDirection::Output=>{let p=egui::pos2(node_rect.right(),oy);oy+=18.0;p}
            };
